        ...
        implements FileChooser
{
     ...
     * contains the path of the folder which files are going to be listed.
     */
    public static final String INPUT_START_FOLDER = "input_start_folder";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if the user is going to select folders instead of select
     * files.
     */
    ...
    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if the user can create files.
     */
    public static final String INPUT_CAN_CREATE_FILES = "input_can_create_files";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a regular expression which is going to be used as a filter to determine which files
     * can be selected.
     */
    ...
    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if only the files that can be selected must be displayed.
     */
    public static final String INPUT_SHOW_ONLY_SELECTABLE = "input_show_only_selectable";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains an instance of the class FileChooserLabels that allows to override the default value
     * of the labels.
     */
    ...
    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if a confirmation dialog must be displayed when creating a
     * file.
     */
    public static final String INPUT_SHOW_CONFIRMATION_ON_CREATE =
            "input_show_confirmation_on_create";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if a confirmation dialog must be displayed when selecting a
     * file.
     */
    public static final String INPUT_SHOW_CONFIRMATION_ON_SELECT =
            "input_show_confirmation_on_select";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if the title must show the full path of the current's
     * folder (true) or only the folder's name (false).
     */
    public static final String INPUT_SHOW_FULL_PATH_IN_TITLE = "input_show_full_path_in_title";

    /**
     * Constant used for represent the key of the bundle object (inside the start's intent) which
     * contains a boolean that indicates if the 'Back' button must be used to navigate to the
     * parents folder (true) or if must follow the default behavior (and close the activity when the
     * button is pressed).
     */
    public static final String INPUT_USE_BACK_BUTTON_TO_NAVIGATE =
    ...
    /**
     * Constant used for represent the key of the bundle object (inside the result's intent) which
     * contains the name of the file that the user wants to create.
     */
...
//        layout.setBackgroundColor(getResources().getColor(R.color.daidalos_backgroud));
        ...
        if (extras != null) {
            ...
            if (extras.containsKey(INPUT_SHOW_CONFIRMATION_ON_CREATE)) {
                core.setShowConfirmationOnCreate(
                        extras.getBoolean(INPUT_SHOW_CONFIRMATION_ON_CREATE));
            }
            if (extras.containsKey(INPUT_SHOW_CONFIRMATION_ON_SELECT)) {
                core.setShowConfirmationOnSelect(
                        extras.getBoolean(INPUT_SHOW_CONFIRMATION_ON_SELECT));
            }
        ...
        core.addListener(
                new FileChooserCore.OnFileSelectedListener()
                {
    ...
    public void onBackPressed()
    {
        ...
        if (!this.useBackButton || current == null || current.getParent() == null ||
                current.getPath().compareTo(this.startFolder.getPath()) == 0) {
            // Close activity.
            super.onBackPressed();
        } else {
            // Open parent.

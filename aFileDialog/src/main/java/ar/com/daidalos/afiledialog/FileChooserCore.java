...
class FileChooserCore
{
    ...
    /**
     * A boolean indicating if only the files that can be selected (they pass the filter) must be
     * show.
     */
    ...
    private View.OnClickListener addButtonClickListener = new View.OnClickListener()
    {
        public void onClick(View v)
        {
            ...
            String title = context.getString(
                    FileChooserCore.this.folderMode
                    ? R.string.daidalos_create_folder
            ...
            String message = context.getString(
                    FileChooserCore.this.folderMode
                    ? R.string.daidalos_enter_folder_name
                                ...
                                int whichButton)
                        {
                            ...
                            if (fileName != null && fileName.length() > 0) {
                                ...
                                FileChooserCore.this.notifyListeners(
                                        FileChooserCore.this.currentFolder, fileName);
                            }
                        }
                    });
     ...
     *
     * @param file
     *         The file or folder selected or the folder in which the file must be created.
     * @param name
     *         The name of the file that must be created or 'null' if a file was selected (instead
     *         of being created).
     */
            ...
            final String name)
    {
        ...
        if ((creation && this.showConfirmationOnCreate ||
                !creation && this.showConfirmationOnSelect)) {
            // Create an alert dialog.
            ...
            String message = null;
            if (FileChooserCore.this.labels != null &&
                    ((creation && FileChooserCore.this.labels.messageConfirmCreation != null) ||
                            (!creation &&
                                    FileChooserCore.this.labels.messageConfirmSelection != null))) {
                message = creation
                          ? FileChooserCore.this.labels.messageConfirmCreation
...
//                            : R.string.daidalos_confirm_select_folder);
                    ...
                    message = context.getString(
                            creation
                            ? R.string.daidalos_confirm_create_file
     ...
     *
     * @param show
     *         'true' for show the confirmation dialog, 'false' for not show the dialog.
     */
     ...
     *
     * @param canCreate
     *         'true' if the user can create files or 'false' if it can only select them.
     */
     ...
     *
     * @param show
     *         'true' if only the files that can be selected must be show or 'false' if all the
     *         files must be show.
     */
    ...
    /**
     * Changes the height of the layout for the buttons, according if the buttons are visible or
     * not.
     */
    private void updateButtonsLayout()
    {
        ...
        addButton.getLayoutParams().width =
                this.canCreateFiles ? ViewGroup.LayoutParams.MATCH_PARENT : 0;
        ...
        okButton.getLayoutParams().width =
                this.folderMode ? ViewGroup.LayoutParams.MATCH_PARENT : 0;
     ...
     * <p/>
     * If no path is specified ('folderPath' is null) the root folder of the SD card is going to be
     * used.
     *
     * @param folderPath
     *         The folder's path.
     ...
     * Loads all the files of a folder in the file chooser.
     * <p/>
     * If no path is specified ('folder' is null) the root folder of the SD card is going to be
     * used.
     *
     * @param folder
     *         The folder.
